...
test.describe("il messaggio di Supabase Auth non arriva a schermo", () => {
  ...
  test("la password sbagliata dà un avviso italiano, e generico", async ({
    page,
  }) => {
  ...
  test("con l’auth irraggiungibile non compaiono né JSON né URL", async ({
    page,
  }) => {
    ...
    await expect(page.locator("body")).not.toContainText(
      /auth\/v1|"status"|service unavailable/i,
    );
  });

  // `abort` fa fallire `fetch`, che è come `auth-js` vede la rete assente:
  // `AuthRetryableFetchError` a `status` 0.
  test("senza rete l’avviso dice di controllare la connessione", async ({
    page,
  }) => {
