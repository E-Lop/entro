...
function payload(over: Record<string, unknown> = {}) {
  ...
  return {
    name: 'Yogurt greco',
    category_id: 'c1',
    expiry_date: tomorrow.toISOString().slice(0, 10),
    storage_location: 'fridge' as const,
    ...over,
  }
}

function quantityError(over: Record<string, unknown>): string | undefined {
...
describe('foodFormSchema — quantità', () => {
  ...
  it('rifiuta una quantità negativa', () => {
    expect(quantityError({ quantity: -1 })).toBeTruthy()
  })
  ...
  it('rifiuta lo zero, che il database non accetta', () => {
    ...
    expect(quantityError({ quantity: 0 })).toBeTruthy()
  })
  ...
  it('rifiuta un positivo che si arrotonda a zero alla precisione della colonna', () => {
    ...
    expect(quantityError({ quantity: 0.004 })).toBeTruthy()
  })
