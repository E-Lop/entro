...
import { pageLabels } from './familyBundle'
...
describe('le etichette dei luoghi dicono quello che dice il bundle', () => {
  ...
  it.each(storageLocationEnum.options)('«%s» ha l’etichetta che il bundle dichiara', (storage) => {
    const expected = pageLabels(PAGE).get(storage)
