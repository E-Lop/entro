/**
 * La tabella dei messaggi degli inviti è **una** decisione in **due** file.
 *
 * `src/lib/inviteErrorMessage.ts` non è un pacchetto condiviso: è una copia, e
 * l'altra è `src/shared/lib/inviteErrorMessage.ts` su entro-mobile. La tabella è
 * stata decisa una volta per i due client (#101, entro-mobile#196), e due
...
const OURS = join(__dirname, '..', 'inviteErrorMessage.ts')
const TWIN = join(TWIN_FOLDER, 'inviteErrorMessage.ts')
...
describe('inviteErrorMessage è uguale sui due client', () => {
  ...
  it('sta confrontando la tabella, non due file svuotati', () => {
    expect(comparable(OURS)).toContain("expired: 'Questo invito è scaduto',")
    expect(
  ...
  it('le due copie dicono le stesse cose', () => {
    expect(
      comparable(OURS),
      'inviteErrorMessage.ts è cambiato da un lato solo: la stessa modifica va portata su ' +
        'entro-mobile (src/shared/lib/inviteErrorMessage.ts), o tolta di qui.',
