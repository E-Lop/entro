...
describe('authErrorMessage', () => {
  it.each([
    ...
    ['over_email_send_rate_limit', 'Troppi tentativi, riprova fra qualche minuto'],
    ['over_request_rate_limit', 'Troppi tentativi, riprova fra qualche minuto'],
    ['otp_expired', 'Il link è scaduto, richiedine uno nuovo'],
  ])('%s → «%s»', (code, message) => {
    expect(authErrorMessage(new AuthApiError('English from the server', 400, code))).toBe(message)
  })

  it('la password debole ha il suo messaggio', () => {
    const error = new AuthWeakPasswordError('Password should be…', 422, ['length'])
  ...
  it('un codice che non è in tabella cade nel generico, mai nel testo del server', () => {
    const error = new AuthApiError('Signups not allowed for this instance', 422, 'signup_disabled')
  ...
  it('uno status 0 senza il nome giusto non basta', () => {
    ...
    expect(authErrorMessage(Object.assign(new Error('x'), { status: 0 }))).toBe(GENERIC_AUTH_ERROR)
  })
  ...
  it('non riconosce un errore dal testo', () => {
    expect(authErrorMessage(new Error('Invalid login credentials'))).toBe(GENERIC_AUTH_ERROR)
    expect(authErrorMessage({ message: 'Invalid login credentials' })).toBe(GENERIC_AUTH_ERROR)
  })

  it.each([[null], [undefined], ['stringa'], [42], [{}], [{ code: 7 }], [{ code: '__proto__' }]])(
    'un valore qualunque (%p) dà il generico senza lanciare',
    (value) => {
